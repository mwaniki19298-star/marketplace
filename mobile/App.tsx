...
async function refreshAuthToken(auth: AuthPayload): Promise<AuthPayload> {
  ...
  try {
    response = await fetch(`${API_BASE}/api/auth/token/refresh/`, {
      method: "POST",
...
async function uploadAssetToCloudinary(asset: ImagePicker.ImagePickerAsset, auth: AuthPayload, type: "listing" | "avatar" | "store_logo" | "store_cover" = "listing"): Promise<string> {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.shell, { paddingTop: insets.top + 8, backgroundColor: theme.background }]}> 
        ...
        {screen === "product" && selected && <ProductScreen theme={theme} listing={selected} saved={savedIds.includes(selected.id)} onToggleSaved={() => toggleSaved(selected.id)} onContactSeller={() => openConversationForListing(selected)} />}
        {screen === "orders" && <OrdersScreen theme={theme} />}
        {screen === "notifications" && <NotificationsScreen theme={theme} />}
...
function ProductScreen({ theme, listing, saved, onToggleSaved, onContactSeller }: { theme: Theme; listing: Listing; saved: boolean; onToggleSaved: () => void; onContactSeller: () => void }) {
  const { width } = useWindowDimensions();
  const [activeImage, setActiveImage] = useState(0);
  const images = listing.images?.length ? listing.images : (listing.image ? [listing.image] : []);
    ...
    <ScreenScroll theme={theme}>
      ...
      <View style={{ paddingHorizontal: 2 }}>
        ...
        <View style={styles.detailButtons}>
          ...
          <Pressable onPress={() => Alert.alert("Purchase request", "Your request has been sent to the seller.")} style={[styles.secondaryButton, { backgroundColor: theme.card, borderColor: theme.border, flex: 1 }]}>
...
function CreateScreen({ theme, auth, onDone }: { theme: Theme; auth: AuthPayload | null; onDone: () => Promise<void> | void }) {
  ...
  const publishListing = async () => {
    ...
    try {
      ...
      const imageUrls = await Promise.all(photos.map(async (photo) => {
        const url = await uploadAssetToCloudinary(photo, auth);
...
function EditProfileSheet({ theme, currentUser, store, insetsBottom, onClose, onUserUpdated, onStoreUpdated }: { theme: Theme; currentUser: ApiUser; store: ProfileStore | null; insetsBottom: number; onClose: () => void; onUserUpdated: (user: ApiUser) => void; onStoreUpdated: (store: ProfileStore) => void }) {
  ...
  async function saveChanges() {
    ...
    try {
      ...
      if (avatar) avatarUrl = await uploadAssetToCloudinary(avatar, activeAuth, "avatar");
      if (logo) logoUrl = await uploadAssetToCloudinary(logo, activeAuth, "store_logo");
      if (cover) coverUrl = await uploadAssetToCloudinary(cover, activeAuth, "store_cover");
...
function OrdersScreen({ theme }: { theme: Theme }) { return <ScreenScroll theme={theme}><View style={styles.pageIntro}><Text style={[styles.pageTitle,{color:theme.text}]}>My orders</Text><Text style={[styles.subtle,{color:theme.muted}]}>Track purchases, delivery progress and completed orders in one place.</Text></View><EmptyState theme={theme} title="Your order history starts here" text="Once you make a purchase, its status, seller details and delivery updates will appear here." icon="orders" /></ScreenScroll>; }
function NotificationsScreen({ theme }: { theme: Theme }) { return <ScreenScroll theme={theme}><View style={styles.pageIntro}><Text style={[styles.pageTitle,{color:theme.text}]}>Notifications</Text><Text style={[styles.subtle,{color:theme.muted}]}>Stay on top of messages, orders, offers and marketplace activity.</Text></View><EmptyState theme={theme} title="You are all caught up" text="New orders, messages, saved-item activity and important Marketplace updates will appear here." icon="notifications" /></ScreenScroll>; }
...
function ListingEditorSheet({ theme, auth, listing, onClose, onSaved, onDeleted, onBoosted }: { theme: Theme; auth: AuthPayload; listing: Listing; onClose: () => void; onSaved: (listing: Listing) => void; onDeleted: (id: string) => void; onBoosted: (listing: Listing) => void }) {
  ...
  const addPhotos = async () => {
    ...
    for (const asset of result.assets) {
      ...
      try {
        const url = await uploadAssetToCloudinary(asset, auth);
        setPhotos((current) => [...current, url].slice(0, 8));
      } catch (e) {
